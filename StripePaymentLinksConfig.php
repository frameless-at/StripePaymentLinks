...
class StripePaymentLinksConfig extends ModuleConfig {
	...
	public function getInputfields() {
		...
		// Webhook Signing Secrets (multiple)
		$fWebhook = $this->modules->get('InputfieldTextarea');
		$fWebhook->name  = 'webhookSecret';
		$fWebhook->label = 'Stripe Webhook Signing Secrets';
		$fWebhook->value = (string)($this->get('webhookSecret') ?? '');
		$fWebhook->notes = 'Enter one secret per line. The module will try each secret until one validates the signature. Required for handling subscriptions.';
		$fWebhook->attr('rows', 4);
		$fs->add($fWebhook);
		...
		$fsW->description = 'Electronic withdrawal function for B2C distance contracts. Delivered as Bootstrap modals on every frontend page.';
			...
			$notify->name  = 'withdrawalNotificationEmail';
			...
			$wd->notes = '{products}, {provider}, {contact_email}, {order_id}, {order_date}, {name}, {email}, {today}, {withdrawal_mail}…{withdrawal_mail_end}, {withdrawal_online}…{withdrawal_online_end}';
			$wd->attr('rows', 8);
